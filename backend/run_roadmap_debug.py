...
log_path = Path(__file__).parent / 'wrap_run_debug_output.txt'

try:
    log_path.write_text('Starting debug runner\n', encoding='utf-8')
    import matplotlib
    matplotlib.use('Agg')
    log_path.write_text(log_path.read_text(encoding='utf-8') + 'Imported matplotlib\n', encoding='utf-8')
    from app.api.roadmap.roadmap_generator import RoadmapVisualGenerator
    from matplotlib.backends.backend_agg import FigureCanvasAgg as FigureCanvas
    import matplotlib.pyplot as plt
    from pathlib import Path

    # Use the JSON payload provided by the user in the request
    data = {
    ...
    log_path.write_text(log_path.read_text(encoding='utf-8') + 'Instantiated data payload\n', encoding='utf-8')

    rg = RoadmapVisualGenerator()
    log_path.write_text(log_path.read_text(encoding='utf-8') + 'Created RoadmapVisualGenerator\n', encoding='utf-8')
    ...
    p = Path(__file__).parent / 'wrap_results_user.txt'
    p.write_text('\n'.join(out_lines), encoding='utf-8')
    log_path.write_text(log_path.read_text(encoding='utf-8') + f'Wrote diagnostics to {p}\n', encoding='utf-8')

    img = rg.generate_roadmap_image(data)
    out = Path('/tmp/test_roadmap_user.png')
    out.write_bytes(img)
    log_path.write_text(log_path.read_text(encoding='utf-8') + f'Wrote image to {out} size={out.stat().st_size}\n', encoding='utf-8')

    log_path.write_text(log_path.read_text(encoding='utf-8') + 'Finished successfully\n', encoding='utf-8')
    print('OK')

except Exception as e:
    tb = traceback.format_exc()
    try:
        log_path.write_text('EXCEPTION:\n' + tb, encoding='utf-8')
    except Exception:
        pass
    print('ERROR')
    sys.exit(1)
